...
# Import optimized Excel indexer
try:
    from src.indexing.optimized_excel_indexer import OptimizedExcelIndexer
    OPTIMIZED_EXCEL_INDEXER_AVAILABLE = True
    logger.info("Optimized Excel indexing available")
except ImportError:
    OPTIMIZED_EXCEL_INDEXER_AVAILABLE = False
    logger.debug("Optimized Excel indexing not available")
...
class MultiAppOpenSearchIndexer:
    ...
    def __init__(self, app_name: Optional[str] = None, 
        ...
        Args:
            ...
            chunking_config: Centralized chunking configuration (optional)
        """
        ...
        self.table_min_rows_per_chunk = chunking_config.table_min_rows_per_chunk
        self.table_max_rows_per_chunk = chunking_config.table_max_rows_per_chunk
        
        # Initialize Semantic Chunker with chunking configuration
        self.semantic_chunker = SemanticChunker(
            chunking_config=chunking_config
        )
        
        logger.info(f"MultiAppOpenSearchIndexer initialized for {self.application_info['name']} - "
                   f"Index: {self.index_name}, chunk_size={self.chunk_size}")
    ...
    def index_document(self, document: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            # Check if this is an optimized Excel document
            if self._is_optimized_excel_document(document):
                return self._index_optimized_excel_document(document)
    ...
    def _is_optimized_excel_document(self, document: Dict[str, Any]) -> bool:
        """Check if document is an optimized Excel document"""
        try:
            ...
            # Check metadata for optimization markers
            metadata = document.get('metadata', {})
            is_optimized = (
                metadata.get('excel_optimization_enabled', False) or
                metadata.get('processing_method') == 'optimized' or
                metadata.get('optimization_enabled', False)
            )
            
            if is_optimized:
                logger.info(f"Detected optimized Excel document: {document.get('file_name', 'unknown')} "
                           f"with {len(optimized_chunks)} chunks")
                return True
            
            return False
            
        except Exception as e:
            logger.error(f"Error checking if document is optimized Excel: {e}")
            return False
    
    def _index_optimized_excel_document(self, document: Dict[str, Any]) -> bool:
        """Index an optimized Excel document using specialized processing"""
        try:
            if not OPTIMIZED_EXCEL_INDEXER_AVAILABLE:
                logger.warning(f"Optimized Excel indexer not available, falling back to standard processing")
                return self._index_excel_document_fallback(document)
            
            # Get Excel optimization configuration
            excel_config = self.rag_config.get('excel_optimization', {})
            batch_size = excel_config.get('batch_size', 1000)
            max_rows_per_chunk = excel_config.get('max_rows_per_chunk', 50)
            
            logger.info(f"Using optimized Excel indexing for {document.get('file_name', 'unknown')}")
            
            # Create optimized Excel indexer
            try:
                optimized_indexer = OptimizedExcelIndexer(
                    app_name=self.app_name,
                    config_path="config/multi_app_config.yaml",
                    excel_batch_size=batch_size,
                    excel_max_rows_per_chunk=max_rows_per_chunk
                )
                
                # Index using optimized method
                success = optimized_indexer.index_excel_document_optimized(document)
                
                if success:
                    optimized_chunks = document.get('optimized_chunks', [])
                    logger.info(f"Successfully indexed optimized Excel document: {document.get('file_name', 'unknown')} "
                               f"with {len(optimized_chunks)} chunks")
                    return True
                else:
                    logger.error(f"Failed to index optimized Excel document: {document.get('file_name', 'unknown')}")
                    return False
                    
            except Exception as indexer_error:
                logger.error(f"Error creating optimized Excel indexer: {indexer_error}")
                logger.info("Falling back to standard Excel processing")
                return self._index_excel_document_fallback(document)
            
        except Exception as e:
            logger.error(f"Error in optimized Excel document indexing: {e}")
            logger.info("Falling back to standard Excel processing")
            return self._index_excel_document_fallback(document)
    
    def _index_excel_document_fallback(self, document: Dict[str, Any]) -> bool:
        """Fallback method to index Excel document using standard processing"""
        try:
            logger.info(f"Using fallback Excel indexing for {document.get('file_name', 'unknown')}")
            
            # Remove optimized chunks to force standard processing
            document_copy = document.copy()
            if 'optimized_chunks' in document_copy:
                del document_copy['optimized_chunks']
            
            # Process as regular document
            return self._index_regular_document(document_copy)
            
        except Exception as e:
            logger.error(f"Error in fallback Excel indexing: {e}")
            return False
    
    def _index_regular_document(self, document: Dict[str, Any]) -> bool:
        """Index a regular (non-optimized Excel) document"""
        try:
            # Check if document has images
            has_images = len(document.get('images', [])) > 0
            is_image_document = document.get('file_extension', '').lower() in ['.png', '.jpg', '.jpeg']
            ...
            if is_image_document and has_images:
                ...
                # Index images as separate multimodal chunks
                images_indexed = 0
                if has_images:
                    images_indexed = self._index_document_images(
                        document, 
            ...
            logger.error(f"Error indexing regular document {document.get('file_name', 'unknown')}: {e}")
            return False
