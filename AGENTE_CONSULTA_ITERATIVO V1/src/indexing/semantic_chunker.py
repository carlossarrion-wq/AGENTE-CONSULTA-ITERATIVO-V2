...
class SemanticChunker:
    def __init__(self, chunking_config: Optional[ChunkingConfig] = None, 
                 chunk_size: int = 4000, chunk_overlap: int = 600, min_chunk_size: int = 250,
                 table_min_rows_per_chunk: int = 5, table_max_rows_per_chunk: int = 20):
        """
        Initialize semantic chunker with chunking parameters.
        
        Args:
            chunking_config: Centralized chunking configuration (preferred)
            chunk_size: Size of each chunk in characters (used if chunking_config not provided)
            chunk_overlap: Overlap between chunks in characters (used if chunking_config not provided)
            min_chunk_size: Minimum chunk size in characters (used if chunking_config not provided)
            table_min_rows_per_chunk: Minimum rows per table chunk (used if chunking_config not provided)
            table_max_rows_per_chunk: Maximum rows per table chunk (used if chunking_config not provided)
        """
        ...
        if chunking_config:
            ...
            logger.info(f"SemanticChunker initialized with centralized config: "
            ...
            logger.info(f"SemanticChunker initialized with parameters: "
                       f"chunk_size={self.chunk_size}, chunk_overlap={self.chunk_overlap}")
