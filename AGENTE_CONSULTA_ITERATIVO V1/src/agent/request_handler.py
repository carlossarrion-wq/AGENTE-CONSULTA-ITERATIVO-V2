...
class RequestHandler:
    ...
    def _format_file_content(self, results: Dict[str, Any]) -> str:
        ...
        if access_mode == 'progressive':
            ...
            # CASO DE ERROR: El archivo no se encontró o hubo un problema
            error_msg = results.get('error', 'Error desconocido')
            formatted += f"❌ **ERROR al recuperar el archivo**: {error_msg}\n\n"
            
            # Si hay archivos disponibles, listarlos para ayudar al usuario
            if 'available_files' in results and results['available_files']:
                available = results['available_files']
                # Filtrar mensajes de error de la lista
                available = [f for f in available if not f.startswith("Error")]
                
                if available:
                    formatted += f"📁 **Archivos disponibles en el índice** (muestra de {len(available)}):\n\n"
                    for i, file_name in enumerate(available[:20], 1):
                        formatted += f"   {i}. {file_name}\n"
                    if len(available) > 20:
                        formatted += f"\n   ... y {len(available) - 20} archivos más\n"
                    formatted += "\n"
            
            formatted += "💡 **Sugerencias**:\n"
            formatted += "   • Verifica que el nombre del archivo sea exacto (mayúsculas, espacios, extensión)\n"
            formatted += "   • Si el archivo tiene caracteres especiales, asegúrate de escribirlos correctamente\n"
            formatted += "   • Puedes usar búsqueda semántica o léxica para encontrar el archivo primero\n"
            
        elif 'content' in results:
            # MODO COMPLETO: Archivo pequeño, enviar contenido completo
            content = results['content']
            file_name = results.get('file_name', results.get('file_path', 'archivo'))
            ...
            # CASO INESPERADO: No hay ni modo progresivo, ni contenido, ni error
            self.logger.error(f"❌ ERROR: _format_file_content recibió results con estructura inesperada")
            ...
            formatted += f"Por favor, contacta con soporte técnico.\n"
