...
def generate_web_crawler_documentation(app_name: str) -> str:
    ...
    Args:
        app_name: Nombre de la aplicación (mulesoft, darwin, sap, saplcorp)
    ...
    try:
        ...
        # Verificar si el web crawler está habilitado para esta aplicación
        config_path = Path("config/web_crawler_config.yaml")
        if not config_path.exists():
            logging.getLogger(__name__).warning(f"Archivo de configuración no encontrado: {config_path}")
            return ""
        
        # Cargar la configuración del web crawler
        ...
        if not enabled:
            logging.getLogger(__name__).info(f"ℹ️  Web crawler deshabilitado para {app_name}")
            return ""
        ...
        logging.getLogger(__name__).info(f"✅ Documentación de web crawler cargada desde {doc_path} para {app_name} (habilitado)")
    ...
    def _load_system_prompt(self, system_prompt_file: str) -> str:
        ...
        try:
            ...
            if "{{WEB_CRAWLER_TOOL}}" in prompt:
                ...
                # Extraer el nombre de la app directamente del nombre del archivo
                import os
                base_filename = os.path.basename(system_prompt_file)
                if base_filename.startswith("system_prompt_") and base_filename.endswith(".md"):
                    app_name = base_filename[len("system_prompt_"):-3]  # Quita "system_prompt_" y ".md"
                else:
