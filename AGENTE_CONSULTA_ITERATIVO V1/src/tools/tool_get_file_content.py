...
class GetFileContent:
    ...
    def _get_all_chunks(self, file_path: str) -> List[Dict]:
        ...
        # CRÍTICO: Normalizar Unicode a NFD (forma descompuesta)
        # Los archivos en OpenSearch están almacenados en NFD
        nfd_path = unicodedata.normalize('NFD', file_path)
        nfd_normalized = unicodedata.normalize('NFD', normalized_path)
        
        # También probar NFC por si acaso
        nfc_path = unicodedata.normalize('NFC', file_path)
        
        # Intentar múltiples estrategias de búsqueda
        # PRIORIDAD: Usar term con file_name (sin .keyword) que es donde está el valor completo
        search_strategies = [
            # Estrategia 1: term con file_name (NFD) - PRIORIDAD MÁXIMA
            {
                "term": {
                    "file_name": nfd_path
                }
            },
            # Estrategia 2: term con file_name (NFC)
            {
                "term": {
                    "file_name": nfc_path
                }
            },
            # Estrategia 3: term con file_name (original)
            {
                "term": {
                    "file_name": file_path
                }
            },
            # Estrategia 4: term con file_name (normalizado NFD)
            {
                "term": {
                    "file_name": nfd_normalized
                }
            },
            # Estrategia 5: match_phrase con file_name
            {
                "match_phrase": {
                    "file_name": nfd_path
                }
            },
            # Estrategia 6: match_phrase con file_name (original)
            {
                "match_phrase": {
                    "file_name": file_path
                }
            },
            # Estrategia 7: Búsqueda con match (más flexible, ignora espacios extras)
            {
                "match": {
                    "file_name": {
                        "query": file_path,
