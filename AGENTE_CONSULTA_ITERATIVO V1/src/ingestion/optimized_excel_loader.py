...
from src.common.chunking_config import ChunkingConfig, get_default_chunking_config
...
class OptimizedExcelLoader:
    ...
    def __init__(self, 
        ...
        Args:
            ...
            batch_size: Number of rows to process in each batch (used if chunking_config not provided)
            max_rows_per_chunk: Maximum rows per semantic chunk (used if chunking_config not provided)
            min_rows_per_chunk: Minimum rows per semantic chunk (used if chunking_config not provided)
            include_headers_in_chunks: Include headers in each chunk for context (used if chunking_config not provided)
            preserve_table_structure: Maintain table formatting in chunks (used if chunking_config not provided)
        """
        # Use centralized config if provided, otherwise use parameters
        if chunking_config:
            self.batch_size = chunking_config.excel_batch_size
            self.max_rows_per_chunk = chunking_config.excel_max_rows_per_chunk
            self.min_rows_per_chunk = chunking_config.excel_min_rows_per_chunk
            self.include_headers_in_chunks = chunking_config.excel_include_headers
            self.preserve_table_structure = chunking_config.excel_preserve_structure
            logger.info(f"OptimizedExcelLoader initialized with centralized config: "
                       f"batch_size={self.batch_size}, max_rows_per_chunk={self.max_rows_per_chunk}")
        else:
            ...
            logger.info(f"OptimizedExcelLoader initialized with parameters: "
                       f"batch_size={self.batch_size}, max_rows_per_chunk={self.max_rows_per_chunk}")
